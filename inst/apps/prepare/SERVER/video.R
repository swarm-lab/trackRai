...
output$videoStatus <- renderUI({
  if (refreshDisplay() > -1 & !isVideoStack(theVideo) & length(input$sortedVideos) == 0) {
    toggleTabs(2:6, "OFF")
...
output$rangeSlider <- renderUI({
  if (refreshVideo() > 0 & isVideoStack(theVideo)) {
    sliderInput("rangePos_x", "Video range",
      width = "100%", min = 1,
...
shinyFileChoose(input, "videoFile_x",
...
observeEvent(input$sortedVideos, {
  ...
  ord <- match(input$sortedVideos, theVideoPath()$name)
  toCheck <- tryCatch({
    test <- sapply(theVideoPath()$datapath[ord], function(path) {
      any(grepl("video", get_content_type(path)))
    })

    if (all(test)) {
      videoStack(unlist(theVideoPath()$datapath[ord]))
    } else {
      NA
    }
  },
  ...
  if (isVideoStack(toCheck)) {
    if (nframes(toCheck) > 0) {
      ...
      bgr1 <<- cloneImage(theImage)
      gray1 <<- extractChannel(theImage, 1)
      gray2 <<- extractChannel(theImage, 1)
      zero <<- zeros(nrow(theImage), ncol(theImage), 3)
      one <<- ones(nrow(theImage), ncol(theImage), 3)
...
observeEvent(refreshDisplay(), {
  if (input$main == "1") {
    if (isImage(theImage)) {
      suppressMessages(write.Image(theImage, paste0(tmpDir, "/display.bmp"), TRUE))
    } else {
...
setBookmarkExclude(c(
  session$getBookmarkExclude(), "refreshVideo", "refreshDisplay",
