...
# UI
shiny::observe({
  if (!is.null(the_model_folder()) & !is.null(video_path()) & trackRcv::is_video_capture(the_video)) {
    ...
    if (!is.na(trackRcv::n_frames(to_check))) {
      ...
      if (!trackRcv::is_image(the_mask)) {
        the_mask <<- reticulate::np_array(
          array(1L, c(trackRcv::n_row(the_image), trackRcv::n_col(the_image), 3)),
          dtype = "uint8"
        )
      }
...
# Load optional mask
shinyFiles::shinyFileChoose(input, "mask_file",
  roots = volumes, session = session,
  defaultRoot = default_root(), defaultPath = default_path()
)
