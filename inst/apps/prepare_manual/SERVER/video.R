...
output$video_status <- shiny::renderUI({
  if (refresh_display() > -1 & !trackRai::trackRcv::is_video_capture(the_video)) {
    .toggleTabs(2:6, "OFF")
    toggled_tabs$toggled[2:6] <<- FALSE
    p("Video missing (and required).", class = "bad")
  } else if (!trackRai::trackRcv::is_video_capture(the_video)) {
    .toggleTabs(2:6, "OFF")
    toggled_tabs$toggled[2:6] <<- FALSE
...
output$display <- shiny::renderUI({
  if (print_display() > 0) {
    if (trackRai::trackRcv::is_image(to_display)) {
      shiny::tags$img(
...
shiny::observeEvent(video_path(), {
  ...
  if (reticulate::py_to_r(to_check$isOpened())) {
    if (!is.na(trackRai::trackRcv::n_frames(to_check))) {
      the_video <<- to_check
...
shiny::observeEvent(input$video_controls_x, {
  if (trackRai::trackRcv::is_video_capture(the_video)) {
